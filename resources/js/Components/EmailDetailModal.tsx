...
export default function EmailDetailModal({ entityType, entityId, emailId, onClose }: Props) {
    ...
    const gmailUrl = email?.gmail_message_id ? `https://mail.google.com/mail/u/0/#inbox/${email.gmail_message_id}` : null;

    return (
        <Modal show={true} onClose={onClose} maxWidth="2xl">
            <div className="flex max-h-[85vh] flex-col">
                {/* Header */}
                <div className="border-gray-200 px-6 py-4 flex items-center justify-between border-b">
                    <div className="gap-3 min-w-0 flex items-center">
                        {activeEmail && (
                            <div
                                className={`p-2 flex-shrink-0 rounded-full ${
                                    activeEmail.direction === 'inbound' ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600'
                                }`}
                            >
                        ...
                        <h3 className="text-lg font-medium text-gray-900 truncate">{activeEmail?.subject || '(No subject)'}</h3>
                    </div>
                    <div className="gap-2 flex flex-shrink-0 items-center">
                        {gmailUrl && (
                            <a
                                ...
                                className="gap-1 rounded-md px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-100 flex items-center"
                                title="Open in Gmail"
                            >
                                <ArrowTopRightOnSquareIcon className="h-4 w-4" />
                                <span className="sm:inline hidden">Open in Gmail</span>
                            </a>
                        )}
                        <button onClick={onClose} className="rounded-md p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-500">
                ...
                <div className="flex-1 overflow-y-auto">
                    {loading ? (
                        <div className="py-12 flex items-center justify-center">
                            <div className="h-6 w-6 animate-spin border-gray-300 border-t-indigo-600 inline-block rounded-full border-2" />
                        </div>
                    ) : error ? (
                        ...
                        <div className="p-6">
                            {/* Thread indicator */}
                            {thread.length > 1 && (
                                <div className="mb-4 gap-1 flex flex-wrap">
                            ...
                            <div className="mb-4 space-y-1 text-sm">
                                <div className="flex">
                                    <span className="w-16 font-medium text-gray-500 flex-shrink-0">From:</span>
                                    <span className="text-gray-900">
                                        {activeEmail.from_name ? (
                                            <>
                                                {activeEmail.from_name} <span className="text-gray-500">&lt;{activeEmail.from_email}&gt;</span>
                                            </>
                                        ) : (
                                ...
                                <div className="flex">
                                    <span className="w-16 font-medium text-gray-500 flex-shrink-0">To:</span>
                                    <span className="text-gray-900">{activeEmail.to_emails?.join(', ') || '-'}</span>
                                </div>
                                {activeEmail.cc_emails && activeEmail.cc_emails.length > 0 && (
                                    <div className="flex">
                                        <span className="w-16 font-medium text-gray-500 flex-shrink-0">Cc:</span>
                                        <span className="text-gray-900">{activeEmail.cc_emails.join(', ')}</span>
                                    </div>
                                )}
                                <div className="flex">
                                    <span className="w-16 font-medium text-gray-500 flex-shrink-0">Date:</span>
                                    <span className="text-gray-900">{formatDateTime(activeEmail.email_date)}</span>
                                </div>
                            </div>
                            ...
                            {activeEmail.has_attachments && activeEmail.attachment_info && activeEmail.attachment_info.length > 0 && (
                                <div className="mb-4 rounded-md bg-gray-50 p-3">
                                    <div className="gap-2 text-sm font-medium text-gray-700 mb-2 flex items-center">
                                    ...
                                    <div className="gap-2 flex flex-wrap">
                                        {activeEmail.attachment_info.map((attachment, index) => (
                                            <div
                                                key={index}
                                                className="gap-1 rounded bg-white px-2 py-1 text-xs text-gray-600 border-gray-200 inline-flex items-center border"
                                            >
                                                <span className="max-w-[150px] truncate">{attachment.name}</span>
                                                {attachment.size && <span className="text-gray-400">({formatFileSize(attachment.size)})</span>}
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="border-gray-200 pt-4 border-t">
                                {activeEmail.body_html ? (
                                    <div
                                        className="prose prose-sm email-content max-w-none"
                                        dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(activeEmail.body_html, { ADD_ATTR: ['target'] }) }}
                                    />
                                ) : activeEmail.body_text ? (
                                    <pre className="text-sm text-gray-700 font-sans whitespace-pre-wrap">{activeEmail.body_text}</pre>
                                ) : (
